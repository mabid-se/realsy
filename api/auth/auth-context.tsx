...
export const MainAuthContextProvider: FC<MainAuthContextProviderProps> = (
    props
) => {
    const router = useRouter();
    const [apiToken, setApiToken] = useState<string | undefined>(
        props.hasInvalidApiToken ? undefined : props.initialApiToken
    );
    const [userProfile, setUserProfile] = useState<UserDto | undefined>(
    ...
    async function signIn(email: string, password: string) {
        ...
        try {
            const authenticateResponse = await axios.post(
                ApiRoutes.BaseUrl + ApiRoutes.UserAuthenticate,
                {
                    identifier: email,
                    password,
                }
        ...
        <AuthContext.Provider
            value={{
                ...
                checkListingFavorited: (listingId: number) => {
                    return favoritedListingIds.some(
                        (favoritedId) => favoritedId === listingId
                    );
                },
                setListingFavorited: async (
                    listingId: number,
                    favorited: boolean
                ) => {
                    await Listings.favoriteListing(listingId, favorited);
                    if (favorited) {
                        setFavoritedListingIds((current) =>
                            current.concat([listingId])
                        );
                    } else {
                        setFavoritedListingIds((current) =>
                        ...
                        onRegisterRequested={async (credentials) => {
                            ...
                            try {
                                const registerResponse = await axios.post(
                                    ApiRoutes.BaseUrl + ApiRoutes.UserRegister,
                                    {
                                ...
                                await hideSignInModal();
                                await router.push(
                                    AppRoute.UserProfile + "?newSignUp=true"
                                );
                            } catch (e) {
                                if (
