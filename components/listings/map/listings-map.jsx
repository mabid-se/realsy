...
const bedOptions = [
    {
        value: "bedroomCount_gte=1",
        label: "Beds: 1+",
    },
    {
        value: "bedroomCount_gte=2",
        label: "Beds: 2+",
    },
    {
        value: "bedroomCount_gte=3",
        label: "Beds: 3+",
    },
    {
        value: "bedroomCount_gte=4",
        label: "Beds: 4+",
    },
    {
        value: "bedroomCount_gte=5",
        label: "Beds: 5+",
    },
    {
        value: "bedroomCount_gte=6",
        label: "Beds: 6+",
    },
    {
        value: "bedroomCount_gte=7",
        label: "Beds: 7+",
    },
    {
        value: "bedroomCount_gte=8",
        label: "Beds: 8+",
    },
];
...
const bathOptions = [
    {
        value: "bathroomCount_gte=1",
        label: "Bathrooms: 1+",
    },
    {
        value: "bathroomCount_gte=2",
        label: "Bathrooms: 2+",
    },
    {
        value: "bathroomCount_gte=3",
        label: "Bathrooms: 3+",
    },
    {
        value: "bathroomCount_gte=4",
        label: "Bathrooms: 4+",
    },
];
const ListingsMap = (props) => {
    ...
    if (minPrice && !minPriceOptions.includes(minPrice)) {
        minPriceOptionsWithAny.push({
            value: minPrice,
            label: "Min: " + formatCurrency(minPrice, { omitDecimal: true }),
        });
    }
    ...
    if (maxPrice && !maxPriceOptions.includes(maxPrice)) {
        maxPriceOptionsWithAny.push({
            value: maxPrice,
            label: "Max: " + formatCurrency(maxPrice, { omitDecimal: true }),
            isDisabled: false,
        });
    }
    maxPriceOptionsWithAny.push({
    ...
    useEffect(() => {
        ...
        if (router.query.minPrice && router.query.maxPrice) {
            ...
            if (minPrice >= maxPrice) {
                pushSoftQueryParams(
                    { minPrice: (maxPrice - 100).toString() },
                    router
                );
            }
        }
    ...
    useEffect(() => {
        ...
        if (router.query.latitude && router.query.longitude) {
            googleMap.setCenter({
                lat: parseFloat(router.query.latitude),
                lng: parseFloat(router.query.longitude),
            });
            ...
            const savedMapState = JSON.parse(
                localStorage.getItem(MAP_STATE_STORAGE_KEY)
            );
            if (savedMapState) {
                googleMap.setCenter({
                    lat: savedMapState.lat,
                    lng: savedMapState.lng,
                });
    ...
    async function loadListingsInViewport() {
        ...
        if (parseInt(router.query.minPrice)) {
            filterString += `&askingPrice_gte=${parseInt(
                router.query.minPrice
            )}`;
        }
        if (parseInt(router.query.maxPrice) && router.query.maxPrice !== "-1") {
            filterString += `&askingPrice_lte=${parseInt(
                router.query.maxPrice
            )}`;
        }
        ...
        if (northEastPoint && southWestPoint) {
            ...
            const listingResult =
                await Listings.findListingsWithinGeographicBounds(
                    {
                    ...
                    router.query.sortField
                        ? `${router.query.sortField}:${router.query.sortOrder}`
                        : "",
                    filterString
                );
    ...
    async function searchAddress(address) {
        ...
        const isPreciseAddress = ["street_address", "premise"].includes(
            location?.types?.[0]
        );
        if (isPreciseAddress) {
            // search was for an individual location, see if an active listing exists
            const streetNumber = location.address_components.filter(
                (component) => component.types?.[0] === "street_number"
            )?.[0].long_name;
            const streetComponent = location.address_components.filter(
            ...
            const activeListingResponse = await ApiClient.get(
                `/listings?${queryString}`
            );
            const activeListings = activeListingResponse.data;
            if (activeListings?.length > 0) {
                pushSoftQueryParams(
                    { viewedListingId: activeListings[0].id },
                    router
                );
            }
        }
            ...
            <div className={styleClasses.listingsMapContainer}>
                ...
                <div className={styleClasses.mobileMapToggle}>
                    <a
                        style={{
                            color:
                                router.query.mobileMap !== "1"
                                    ? "#fff"
                                    : Colors.lightBlue,
                        }}
                        onClick={() =>
                            clearSoftQueryParams(["mobileMap"], router)
                        }
                        children={"List"}
                    />
                    <a
                        ...
                        onClick={() =>
                            pushSoftQueryParams({ mobileMap: "1" }, router)
                        }
                        children={"Map"}
                    />
                </div>
                        ...
                        <Grid item xs={6} md={3}>
                            ...
                            >
                                <Grid item>
                                    <Typography
                                        variant="subtitle1"
                                        className={styleClasses.inputLabel}
                                    >
                                        Price Min
                                    </Typography>
                                </Grid>
                        ...
                        >
                            {(router.query.mobileMap !== "1" ||
                                ...
                                <>
                                    ...
                                    <ul
                                        className={
                                            styleClasses.sortControlsContainer
                                        }
                                        style={{ display: "none" }}
                                    >
                                        {[
                                            ...
                                            return (
                                                <li
                                                    key={field.code}
                                                    className={`${styleClasses.sortControl} ${field.code}`}
                                                    onClick={() => {
                                                        if (
                                                            router.query
                                                                .sortField !==
...
ListingsMap.defaultProps = {
    ...
    minPriceOptions: [100000, 200000, 300000, 400000, 500000, 600000],
    maxPriceOptions: [
