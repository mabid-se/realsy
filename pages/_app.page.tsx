...
const CustomApp = (props: CustomAppProps) => {
    ...
    const getLayout =
        ...
        (props.Component as AppPage).defaultLayout ??
        ((page: any) => page);
    ...
    return (
        <>
            <Head>
                <title>
                    {AppInfo.name}: {AppInfo.defaultPageTitle}
                </title>
                <meta
                    name="viewport"
...
CustomApp.getInitialProps = async (appContext: AppContext) => {
    ...
    if (!process.browser) {
        const globalContentResponse = await CmsClient.get(
            CmsRoutes.globalContent
        );
    ...
    if (
        (appContext.Component as AppPage).requiresAuth &&
        (!apiToken || hasInvalidApiToken)
    ) {
        redirect(
            `/?showSignIn=true&signInRedirect=${appContext.ctx.req?.url
                ?.replace("?", "(q)")
                .replace("&", "(a)")}`,
            appContext.ctx.res,
            302
