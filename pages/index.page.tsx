    ...
    useEffect(() => {
        if (props.failedSignUp) {
            NotificationManager.error(
                props.userNotification ? props.userNotification : null,
                "Failed sign up"
            );
        }
    }, []);
    ...
    return (
        <>
            <Head>
                <title>
                    {AppInfo.name}:{" "}
                    {props.htmlTitle ?? AppInfo.defaultPageTitle}
                </title>
            </Head>
            <div data-testid={"main-content"} className={styleClasses.homePage}>
...
HomePage.getInitialProps = async (ctx: NextPageContext) => {
    const homePageResponse = await CmsClient.get(
        `${CmsRoutes.contentPage}/homepage`
    );
